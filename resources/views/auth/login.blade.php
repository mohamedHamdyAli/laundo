...
<body>
    <div id="auth" class="login_bg" style="background-image: url('{{ $login_bg_image ?? '' }}');">
        <img src="{{ $login_bg_image ?? '' }}"
            data-custom-image="{{ getSettingValue('Login_Cover') ? asset('storage/' . getSettingValue('Login_Cover')) : asset('assets/images/bg/login.jpg') }}"
            alt="" style="display: none" id="bg_image">
            ...
            <div class="col-lg-3 col-12 card" id="auth-box">
                <div class="auth-logo mb-5 d-block">
                    <img id="company_logo" src="{{ $company_logo ?? '' }}"
                        data-custom-image="{{ asset('storage/' . getSettingValue('App_Logo')) }}" alt="Logo">
                </div>
                        ...
                        <form method="POST" action="{{ route('login') }}" id="frmLogin">
                            ...
                            @if (config('app.demo_mode'))
                                <div class="text-danger text-center mt-2" role="alert">
                                    ...
                                    <br><a class="text-decoration-underline" target="_blank"
                                        href="{{ Request::root() }}">{{ Request::root() }}</a>
                                </div>
                            @endif
                            @if (config('app.demo_mode'))
                                <div class="row mt-3">
