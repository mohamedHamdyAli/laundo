...
("الطلبات", "Orders", "الويزارد والقايمة والتتبع.", [
  ...
  ("GET", "/orders/{id}/track", "customer", "الخط الزمني وكارت المندوب.", [],
   ["الخط الزمني <strong>ثمان خطوات</strong>. ستة منهم بتتقرا من <code>order_status_logs</code> — يعني الخطوة تفضل متعدّاة بعد ما الطلب يتجاوزها — واتنين متولّدين: <code>driver_on_way</code> حالة حقيقية كانت متسجّلة ومترسمتش، و<code>out_for_delivery</code> <strong>مش حالة أصلاً</strong>.",
    ...
    "<code>pickup_location</code> و<code>delivery_location</code> هما البابين كـ<code>{lat, lng}</code>، جنب <code>driver.location</code> اللي هي النقطة الحية. النقطة لوحدها على ماب فاضية مكنتش بتقول للعميل العربية قربت من شارعه ولا لأ.",
